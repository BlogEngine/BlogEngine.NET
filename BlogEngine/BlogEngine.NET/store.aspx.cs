...
/// <summary>
/// ICallbackEventHandler required for store.js WebForm_DoCallback.  Called
/// by button click client side code store.js: beginSendMessage function
/// </summary>
public partial class store : BlogBasePage, ICallbackEventHandler
{
    ...
    public void RaiseCallbackEvent(string eventArgument)
    {
        ...
        if (arg.Length >= 5)
        {
            ...
            if (UseCaptcha)
            {
                if (HasSpamSigns(message) || HasHigherAsciiValues(message))
                {
                    _Callback = $"<p>{BlogSettings.Instance.ContactErrorMessage}</p>" +
                        $"Subject: <b>{subject}</b><br>" +
                        $"From: <b>{email}</b>" +
                        $"<p><textarea>{message}</textarea><p>" +
                        $"<a href='mailto:?subject={subject}&body={message}'>Try emailing " +
                        $"it manually</a> to the following address:" +
                        $"<p><img src='/content/images/blog/email-address.png'/></p>";
                    return;
                }
            }
    ...
    private bool HasSpamSigns(string message)
    {
        var signs = BlogSettings.Instance.FacebookAppId.Split(',');
        if (signs.Length > 0)
        {
            foreach (var sign in signs)
                if (message.Contains(sign.Trim(), StringComparison.InvariantCultureIgnoreCase))
                    return true;
        }
        return false;
    }
    private bool HasHigherAsciiValues(string message)
    {
        var result = message.ToCharArray().Any(c => (int)c > 255);
        return result;
    }
    // ICallbackEventHandler Members END OF CODE
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
